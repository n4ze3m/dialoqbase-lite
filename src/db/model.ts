...
export class DialoqAiModel {
    ...
    async setModelsOrIgnore(models: Model[]): Promise<void> {
        ...
        const newModels = models.filter((m) => !existingModels.some((em) => em.model_id === m.model_id))
...
export const getAllModels = async ({ hidden }: { hidden?: boolean }) => {
    const model = new DialoqAiModel()
    const models = await model.getModels()
    if (hidden === undefined) {
        return models
    }
    return models.filter((m) => m.hidden === hidden)
}
...
export const upsertModels = async (models: ModelConfig[]) => {
    const model = new DialoqAiModel()
    return model.setModelsOrIgnore(models.map((m) => ({ ...m, createdAt: new Date().toISOString(), hidden: false })))
}